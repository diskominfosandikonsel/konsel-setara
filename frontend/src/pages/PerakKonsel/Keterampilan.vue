        ...
        <q-header bordered class="bg-white text-black">
            <q-toolbar>
                <q-toolbar-title>
                    ...
                    <span class="headerText">Keterampilan</span>
                </q-toolbar-title>
            </q-toolbar>
        </q-header>
        ...
        <q-page-container>
            <q-page class="q-pa-md" style="background-color: #FFFFFF;">
                ...
                <q-list class="mulish q-pa-md">
                    <template v-if="perak.keterampilan && perak.keterampilan.length > 0">
                        <q-card v-for="(data) in perak.keterampilan" :key="data.id" class="q-mb-md cardPendidikan shadow-2">
                            <q-card-section class="q-pa-md">
                                <div class="row no-wrap items-start justify-between">
                                    <div class="col">
                                        <div class="text-weight-bold textPrimary" style="font-size: 16px;">{{ data.uraian_keterampilan }}</div>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            ...
            const payloadKeterampilan = {
                biodata_id: this.perak.biodata[0].id,
                data_ke: this.page_first,
                cari_value: this.cari_value,
                limit: this.page_limit
            }
            await this.perak.fetchKeterampilan(payloadKeterampilan);
        },
        ...
        confirmDelete(data) {
            this.$q.dialog({
                ...
                await this.perak.removeKeterampilan(data.id);
                this.loadData();
            })
        },
    },
