  ...
  <q-layout view="hHh lpR fFf" style="background-color: #f6f6f6">
    <q-header bordered class="bg-white text-black">
      <q-toolbar>
        <q-toolbar-title>
          ...
          <span class="sapa_title">Data Aksi Perubahan</span>
        </q-toolbar-title>
      </q-toolbar>
    </q-header>
    ...
    <q-page-container>
      <q-page class="q-pa-md mulish">
        ...
        <div v-else-if="aksiList.length">
          <div class="row q-col-gutter-sm q-mt-sm">
            <div class="col-12" v-for="item in aksiList" :key="item.id">
              <q-card
                class="aksi-card cursor-pointer"
                clickable
                v-ripple
                @click="goDetail(item)"
              >
          ...
          <q-card class="bg-white column fit">
            ...
            <q-toolbar class="bg-white toolbar-bordered">
              ...
              <q-toolbar-title class="text-subtitle2 text-weight-medium">
                {{ selectedItem?.judul || "Detail Aksi Perubahan" }}
              </q-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    async loadData(reset = false) {
      ...
      await this.erida.fetchAksi(payload, !reset);

      const totalPage = this.erida.dataLastPage || 1;
      this.lastPage = totalPage;

      this.aksiList = [...this.erida.aksi];
