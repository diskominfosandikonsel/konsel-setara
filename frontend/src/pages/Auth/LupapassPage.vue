    ...
    <div class="form-section">
      ...
      <transition name="fade-slide" mode="out-in">
        <div v-if="!emailSent" key="step1" class="step-wrapper">
          ...
          <div class="input-group">
            ...
            <div class="input-wrapper" :class="{ 'input-error': emailError }">
              <svg class="input-icon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <input type="email" placeholder="Ketikkan Email Anda" v-model="form.email" class="custom-input"
                @input="emailError = ''" />
            </div>
        ...
        <div v-else key="step2" class="step-wrapper">
          <!-- Kode Reset -->
          <div class="input-group">
            <label class="input-label">Kode OTP (Cek Email)</label>
            <div class="input-wrapper" :class="{ 'input-error': kodeError }">
              <svg class="input-icon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4" />
              </svg>
              <input type="text" placeholder="Masukkan Kode" v-model="form.kode" class="custom-input"
                @input="kodeError = ''" />
            </div>
            <span v-if="kodeError" class="error-text">{{ kodeError }}</span>
          </div>
          ...
          <div class="input-group">
            ...
            <div class="input-wrapper" :class="{ 'input-error': passError }">
              ...
              <input :type="showPassword ? 'text' : 'password'" placeholder="**********" v-model="form.password"
                class="custom-input" @input="passError = ''" />
              <button class="toggle-password" @click="showPassword = !showPassword" type="button">
                <svg v-if="!showPassword" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24"
          ...
          <div class="input-group">
            ...
            <div class="input-wrapper" :class="{ 'input-error': form.konfirmasi && form.password !== form.konfirmasi }">
              ...
              <input :type="showKonfirmasi ? 'text' : 'password'" placeholder="**********" v-model="form.konfirmasi"
                class="custom-input" />
              <button class="toggle-password" @click="showKonfirmasi = !showKonfirmasi" type="button">
                <svg v-if="!showKonfirmasi" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
...
export default {
  ...
  methods: {
    async sendReset() {
      ...
      this.isLoading = true
      try {
        await AuthService.forgotPassword({ email: this.form.email })
        this.emailSent = true
        Notify.create({ message: 'Kode OTP telah dikirim ke email Anda', color: 'positive' })
      } catch (error) {
        this.errorMsg = error?.response?.data?.message || 'Gagal mengirim kode reset. Coba lagi.'
      } finally {
    ...
    async doReset() {
      ...
      if (!this.form.kode) {
        this.kodeError = 'Kode OTP wajib diisi.'
        return
      }
      ...
      try {
        ...
        Notify.create({ message: 'Password berhasil diperbarui, silakan login', color: 'positive' })
        this.$router.push('/login')
      } catch (error) {
        this.errorMsg = error?.response?.data?.message || 'Gagal memperbarui password. Coba lagi.'
      } finally {
        this.isLoading = false
      }
    }
